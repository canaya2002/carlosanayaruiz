...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return generatePageMetadata({
    title: locale === 'en'
      ? 'Books – Technical Knowledge in Book Format'
      : 'Libros – Conocimiento Técnico en Formato Libro',
    description: locale === 'en'
      ? 'Carlos Anaya Ruíz — Books on technical SEO, Next.js, web performance, and structured data. Practical guides for developers and SEO consultants.'
      : 'Carlos Anaya Ruíz — Libros sobre SEO técnico, Next.js, rendimiento web y datos estructurados. Guías prácticas para desarrolladores y consultores SEO.',
      ...
      <div className="mx-auto max-w-4xl">
            ...
            <Card key={book.id} className="overflow-hidden">
              <div className="md:flex">
                <div className="relative h-64 overflow-hidden bg-muted md:h-auto md:w-56 md:shrink-0">
                  <Image
                    src={book.coverImage}
                    alt={`${book.title} — libro de Carlos Anaya Ruíz sobre ${book.tags[0]}`}
                    fill
                ...
                <div className="flex flex-1 flex-col p-6">
                  ...
                  <p className="mb-4 text-sm font-medium text-primary">{book.subtitle}</p>
                  ...
                  <div className="mt-auto flex flex-wrap items-center gap-4">
                    ...
                    {book.pages && <span className="text-sm text-muted-foreground">{book.pages} {t('pages')}</span>}
                  </div>
                  <div className="mt-4">
                    <Button asChild className="gap-2">
                      <a href={book.purchaseUrl} target="_blank" rel="noopener noreferrer">
                        {t('buy')}<ExternalLink className="h-4 w-4" />
                      </a>
                    </Button>
                  </div>
                </div>
        ...
        <div className="mt-12 rounded-lg border bg-card p-8 text-center">
          ...
          <p className="mb-6 text-muted-foreground">
            {locale === 'en' ? 'Beyond books, I offer consulting and development services.' : 'Más allá de los libros, ofrezco consultoría y desarrollo.'}
          </p>
          <Button variant="outline" asChild className="gap-2">
            <Link href="/contacto">
              {locale === 'en' ? 'Contact me' : 'Contáctame'}<ArrowRight className="h-4 w-4" />
            </Link>
          </Button>
        </div>
      </div>
